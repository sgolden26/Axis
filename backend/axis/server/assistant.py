...

{
  ...
  "edits": [<edit objects, optional, max 3>],
  "rationale": "<short prose>"
}

Each edit object uses one of these shapes. Spawned ids are generated by
the simulator and returned to the client. You may issue orders in the
same response targeting the spawned platform IFF the LLM's `order_id`
references the spawn placeholder via `<spawn_target_id>` strings, BUT
the simpler pattern is to just spawn this round and orchestrate orders
the next round.
