...
- Use existing ids from the brief verbatim. Coordinates are
  `[lon, lat]` in degrees.
- Targets for engage / strikes must belong to enemy factions. SEAD sorties
  must target a `missile_range` whose `category` is `sam`.
- Prefer 3-8 orders. Concentrate force; do not spread thin.
...
def suggest_orders(
    *,
    ...
    return {"orders": kept, "rationale": rationale, "warnings": warnings}
