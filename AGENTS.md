...
- `frontend/src/ui/` HUD, `LeftDock` (FilterChips + OOBTree), `RightPanel` (Context tab wrapping Sidebar / Decision tab wrapping DecisionEngine), CountryRoster, Bottom bar (MeasureTool, Bookmarks, EventTicker), Minimap, HoverCard, KeyboardShortcuts. Sidebar holds the per-kind detail panels for every selectable entity (city, unit, territory, country, oblast, depot, airfield, naval base, crossing, supply, ISR, missile, AOR, frontline).
- `frontend/src/state/store.ts` Zustand store (selection, hover, visible layers, right tab, left/right panel open, OOB expansion, bookmarks, measure path, ticker pause, help overlay, choropleth metric).
